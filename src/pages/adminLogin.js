  ...
  window.handleAdminLogin = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        error.style.display = 'block';
        btn.disabled = false;
        btn.innerHTML = '<i data-lucide="shield" style="width:18px;"></i> Admin Sign In';
        if (window.lucide) window.lucide.createIcons();
        return;
      }

      // Save JWT and user — mark as admin session
      localStorage.setItem('ck_admin_token', data.token);
      localStorage.setItem('ck_admin_user', JSON.stringify(data.user));
      ...
      // Short success feedback before redirect
      btn.innerHTML = '✓ Access Granted';
      btn.style.background = 'linear-gradient(135deg, #00c896, #00e676)';
      setTimeout(() => {
      ...
      <div class="admin-login-wrap">
        ...
        <div class="admin-card glass-card reveal delay-1">
          ...
          <div class="admin-card-header">
            ...
            <p class="admin-card-sub">CircuitKart — Backend Administration</p>
          </div>
          ...
          <form onsubmit="handleAdminLogin(event)">
            ...
            <div class="admin-field">
              <label class="admin-label">
                <i data-lucide="mail" style="width:14px;"></i> Admin Email
              </label>
              <div class="auth-input-wrap">
                <i data-lucide="mail" class="auth-input-icon"></i>
                <input id="admin-email" type="email" class="auth-input admin-input"
                  placeholder="admin@circuitkart.com" required autocomplete="email">
              </div>
            </div>

            <div class="admin-field">
              <label class="admin-label">
                <i data-lucide="lock" style="width:14px;"></i> Admin Password
              </label>
              <div class="auth-input-wrap">
                <i data-lucide="lock" class="auth-input-icon"></i>
                <input id="admin-password" type="password" class="auth-input admin-input"
                  placeholder="Enter admin password" required autocomplete="current-password">
                <button type="button" class="auth-eye-btn" onclick="toggleAdminPassword()">
                  <i id="admin-eye-icon" data-lucide="eye"></i>
                </button>
              </div>
            </div>

            <button id="admin-login-btn" type="submit" class="admin-submit-btn">
          ...
          <div class="admin-card-footer">
            <i data-lucide="info" style="width:13px; color: var(--text-tertiary);"></i>
            <span>This portal is for authorized administrators only. All access attempts are logged.</span>
          </div>
        </div>

        <!-- Back link -->
        <a href="#/login" class="admin-back-link reveal delay-2">
          <i data-lucide="arrow-left" style="width:14px;"></i> Back to Customer Login
        </a>
      </div>
    </div>
  `;
