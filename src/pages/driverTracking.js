...
export async function DriverTrackingPage(container, params) {
  ...
  const token = localStorage.getItem('ck_token') || localStorage.getItem('ck_admin_token');
  ...
  container.innerHTML = `
    <div id="driver-page" style="min-height:100vh;background:linear-gradient(135deg,#0a0a0f 0%,#0d1117 50%,#0a0f1a 100%);display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:2rem 1rem;font-family:'Inter',sans-serif;">
      ...
      <div id="driver-order-card" style="width:100%;max-width:500px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:1.25rem;margin-bottom:1.25rem;backdrop-filter:blur(12px);">
        ...
        <div style="font-size:1.1rem;font-weight:700;color:#00d4ff;font-family:'JetBrains Mono',monospace;" id="driver-order-id">Loading...</div>
      ...
      <div id="gps-status-card" style="width:100%;max-width:500px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:1.5rem;margin-bottom:1.25rem;backdrop-filter:blur(12px);">
        ...
        <div id="coords-display" style="background:rgba(0,0,0,0.3);border-radius:10px;padding:1rem;margin-bottom:1.25rem;min-height:70px;display:flex;flex-direction:column;justify-content:center;">
          <div style="color:#475569;font-size:0.75rem;margin-bottom:0.4rem;font-family:'JetBrains Mono',monospace;">CURRENT POSITION</div>
          <div id="lat-display" style="color:#94a3b8;font-family:'JetBrains Mono',monospace;font-size:0.88rem;">Latitude: —</div>
          <div id="lng-display" style="color:#94a3b8;font-family:'JetBrains Mono',monospace;font-size:0.88rem;">Longitude: —</div>
          <div id="acc-display" style="color:#475569;font-family:'JetBrains Mono',monospace;font-size:0.75rem;margin-top:0.25rem;">Accuracy: —</div>
        </div>
        ...
        <div id="gps-error" style="display:none;background:rgba(239,68,68,0.1);border:1px solid rgba(239,68,68,0.3);border-radius:10px;padding:1rem;margin-bottom:1rem;">
          <div style="color:#ef4444;font-weight:600;font-size:0.88rem;margin-bottom:0.25rem;">⚠️ GPS Error</div>
          <div id="gps-error-msg" style="color:#fca5a5;font-size:0.8rem;"></div>
        </div>

        <!-- Control Buttons -->
        <div style="display:flex;gap:10px;">
          <button id="btn-start-gps" onclick="window.driverStartGPS()" style="flex:1;padding:14px;background:linear-gradient(135deg,#00d4ff,#0ea5e9);color:#000;border:none;border-radius:12px;font-weight:700;font-size:0.95rem;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;">
            📍 Start Tracking
          </button>
          <button id="btn-stop-gps" onclick="window.driverStopGPS()" style="flex:1;padding:14px;background:rgba(239,68,68,0.15);color:#ef4444;border:1px solid rgba(239,68,68,0.4);border-radius:12px;font-weight:700;font-size:0.95rem;cursor:pointer;transition:all 0.2s;display:none;align-items:center;justify-content:center;gap:8px;">
            ⏹ Stop Tracking
          </button>
        </div>
      </div>

      <!-- Manual Coords Fallback (shown when GPS denied) -->
      <div id="manual-coords-panel" style="display:none;width:100%;max-width:500px;background:rgba(245,158,11,0.07);border:1px solid rgba(245,158,11,0.3);border-radius:16px;padding:1.25rem;margin-bottom:1.25rem;backdrop-filter:blur(12px);">
        <h4 style="color:#f59e0b;font-size:0.88rem;font-weight:700;margin-bottom:0.75rem;">📍 Manual Location Entry</h4>
        <p style="color:#94a3b8;font-size:0.78rem;margin-bottom:1rem;">GPS permission was denied. Enter your coordinates manually from Google Maps or another app.</p>
        <div style="display:flex;gap:10px;margin-bottom:10px;">
          <div style="flex:1;">
            <label style="color:#64748b;font-size:0.72rem;display:block;margin-bottom:4px;">Latitude</label>
            <input id="manual-lat" type="number" step="any" placeholder="13.0827" style="width:100%;padding:10px;background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.15);border-radius:8px;color:#fff;font-size:0.88rem;box-sizing:border-box;" />
          </div>
          <div style="flex:1;">
            <label style="color:#64748b;font-size:0.72rem;display:block;margin-bottom:4px;">Longitude</label>
            <input id="manual-lng" type="number" step="any" placeholder="80.2707" style="width:100%;padding:10px;background:rgba(0,0,0,0.4);border:1px solid rgba(255,255,255,0.15);border-radius:8px;color:#fff;font-size:0.88rem;box-sizing:border-box;" />
          </div>
        </div>
        <button onclick="window.driverSendManual()" style="width:100%;padding:11px;background:rgba(245,158,11,0.2);color:#f59e0b;border:1px solid rgba(245,158,11,0.4);border-radius:10px;font-weight:600;cursor:pointer;font-size:0.88rem;">
      ...
      <div style="width:100%;max-width:500px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:14px;padding:1.25rem;">
        <h4 style="color:#64748b;font-size:0.78rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.75rem;">How it works</h4>
        <div style="display:flex;flex-direction:column;gap:8px;">
          <div style="display:flex;gap:10px;align-items:flex-start;">
            <span style="font-size:1rem;flex-shrink:0;">1️⃣</span>
            <p style="color:#64748b;font-size:0.8rem;margin:0;">Tap <strong style="color:#94a3b8;">Start Tracking</strong> and allow location permission when prompted.</p>
          </div>
          <div style="display:flex;gap:10px;align-items:flex-start;">
            <span style="font-size:1rem;flex-shrink:0;">2️⃣</span>
            <p style="color:#64748b;font-size:0.8rem;margin:0;">Your GPS location is sent to the server every 5 seconds automatically.</p>
          </div>
          <div style="display:flex;gap:10px;align-items:flex-start;">
            <span style="font-size:1rem;flex-shrink:0;">3️⃣</span>
            <p style="color:#64748b;font-size:0.8rem;margin:0;">The customer and admin can see your live position on the map.</p>
          </div>
          <div style="display:flex;gap:10px;align-items:flex-start;">
            <span style="font-size:1rem;flex-shrink:0;">4️⃣</span>
            <p style="color:#64748b;font-size:0.8rem;margin:0;">Tap <strong style="color:#94a3b8;">Stop Tracking</strong> when the delivery is complete.</p>
          </div>
        </div>
      </div>
    </div>`;
  ...
  async function loadOrderData() {
    try {
      ...
      if (res.ok) {
        ...
        const addrParts = [info.address1, info.city, info.state, info.zip].filter(Boolean);

        document.getElementById('driver-order-id').textContent = orderId;
        document.getElementById('driver-order-status').textContent = `Status: ${(order.status || '').replace(/_/g, ' ')}`;
        document.getElementById('driver-delivery-addr').textContent = addrParts.length ? `📍 ${addrParts.join(', ')}` : 'Delivery address not available';
      } else {
        // Try admin endpoint
        const adminRes = await fetch(`/api/admin/orders/${orderId}`, { headers: { Authorization: `Bearer ${token}` } });
        if (adminRes.ok) {
          ...
          document.getElementById('driver-order-id').textContent = orderId;
          document.getElementById('driver-order-status').textContent = 'Order details unavailable';
        }
      }
    } catch (e) {
      document.getElementById('driver-order-id').textContent = orderId;
    ...
    // Fetch assignment
    try {
      const aRes = await fetch(`/api/tracking/assignment/${orderId}`, { headers: { Authorization: `Bearer ${token}` } });
      if (aRes.ok) {
        ...
        if (a?.driver_name) {
          ...
          badge.querySelector('span').textContent = `👤 ${a.driver_name}`;
        }
      }
    } catch (e) {}
  }
  ...
  socketManager._socket?.emit('driver:join_order', { orderId });
  ...
  function sendLocation(lat, lng) {
    ...
    // Update display
    document.getElementById('lat-display').textContent = `Latitude:  ${lat.toFixed(6)}`;
    document.getElementById('lng-display').textContent = `Longitude: ${lng.toFixed(6)}`;
    document.getElementById('last-update').textContent = `Last update: ${new Date().toLocaleTimeString('en-IN')}`;
  ...
  window.driverStartGPS = () => {
    if (!navigator.geolocation) {
      ...
      document.getElementById('manual-coords-panel').style.display = 'block';
      return;
    }
    ...
    watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        document.getElementById('acc-display').textContent = `Accuracy: ±${Math.round(accuracy)}m`;
      ...
      (err) => {
        ...
        switch (err.code) {
          case err.PERMISSION_DENIED:
            msg = 'Location permission denied. Please allow location access in browser settings, or use manual entry below.';
            document.getElementById('manual-coords-panel').style.display = 'block';
            break;
          case err.POSITION_UNAVAILABLE:
            msg = 'GPS signal unavailable. Please move to an open area or use manual entry.';
            document.getElementById('manual-coords-panel').style.display = 'block';
            break;
          case err.TIMEOUT:
  ...
  window.driverStopGPS = () => {
    ...
    if (socketManager._socket?.connected) {
      socketManager._socket.emit('driver:go_offline', { orderId });
    }
  };
  ...
  window.driverSendManual = () => {
    ...
    document.getElementById('gps-label').textContent = 'Manual Location Sent';
    document.getElementById('gps-dot').style.background = '#f59e0b';
    document.getElementById('broadcast-badge').style.display = 'block';
    document.getElementById('last-update').textContent = `Manual update: ${new Date().toLocaleTimeString('en-IN')}`;
  };
